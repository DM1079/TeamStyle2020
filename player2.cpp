...
int gotosubmit(Dishpos i)//return1成功提交，return - 1放回原处,return 2没捡到菜，return-1放回原处
{
    ...
    if (PlayerInfo.tool != Condiment) {//如果手上没有调料，捡起来试试
        mystorage.updatestorage();//这个参数没用了，主要看有没有Condiment
        if (mystorage.condimentList.empty() == FALSE) 
        {
...
//返回2表示捡到成品食材，返回1表示送到主灶台，否则无事发生
{
    ...
    if (PlayerInfo.dish >= Flour && PlayerInfo.dish <= Ketchup)
    {
        return 1;//送回主label灶台
    }
    if (PlayerInfo.dish <Flour && res==1)//指捡到了普通菜，丢到灶台里烧了
    {
...
int throw_darkdish2(int _label)//0:有人在做菜，看看周围有没有可以捡走的菜；1：空灶台；0，1调用trick就好；2：准备提交成品菜
{
    ...
    for (list<Obj>::iterator i = l.begin(); i != l.end(); i++)
    {
        if (i->blockType == 3 && i->dish != 0 && i->dish < OverCookedDish)//如果发现灶台里面有菜,而且不是overcook或者黑暗料理
        {
            ...
            {
                cout << "throw dark dish  " << PlayerInfo.dish << endl;
                put(2, angle_4[cooklabel[_label][3]], TRUE);
                //放回灶台里 为难一下别人嘛
                return 1;//用这个灶台捣乱
            }
            else //不是黑暗料理！捡到宝了！
...
void play() {
    ...
    //只在第一次的时候跑到最远的食物生成点般一个食物，等label更新之后送过去。
        ...
        int randl = rand() % 2;
        int randlabel = label;//随机去某个灶台捣乱。1234 +-1
        if (randl == 0)
        {
            randlabel--;
        }
