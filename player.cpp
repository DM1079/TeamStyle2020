...
public:
    ...
    int getCnt(DishType _dishType)
    {
        return mstorage[int(_dishType)].cnt;
    }
...
Astar astar;
static map<char, Protobuf::Direction> ctodire{
...
void move_dir(char c)
{
    ...
    cout << "move :" << c <<"  time :"<<getGameTime()<<endl;
    if (c == 's')
        return;
    if (speed == 4)
    {
...
double calcdis(Point end)
{
    ...
    return sqrt(((double)end.x + 0.5 - x) * ((double)end.x + 0.5 - x) +
        ((double)end.y + 0.5 - y) * ((double)end.y + 0.5 - y));
}
double calcdis(Point point, Point end)
{
    //用简单的欧几里得距离计算H，这个H的计算是关键，还有很多算法，没深入研究^_^
...
{
    //找第二近的食物生成点
    Point Point1(25, 4), Point2(42, 39),
        Point3(7, 40);  //三个食物生成点下方一格，这样统一再往上走一步就好
    Point Pos(PlayerInfo.position.x, PlayerInfo.position.y);
...
{           // getdish=1~8表示沿路定向捡食材，-1表示沿路随机捡
    ...
    cout << "speed" << PlayerInfo.moveSpeed << x<<','<<y<<endl;
    ...
    while (1)
    {
        ...
        for (auto& lp : astar.GetPath(x, y, false))
        {
            ...
            if (astar.maze[x][y] == 1)  //重新检测目标是否是障碍物，找dest周围的点
            {
                ...
                x = posnear.x;
                y = posnear.y;
                cout << " meet people break for" << endl;
                break;  // break for循环
            }
        }
        if ((int)PlayerInfo.position.x == (int)x &&
            (int)PlayerInfo.position.y == (int)y)
        {
...
//走到灶台后第一件事检查黑暗料理
int throw_darkdish(
    int _label)  //检查灶台上有无黑暗料理，如果有，尝试拾取，拾取成功就扔掉，拾取失败就标记这个灶台不行，找下一个灶台tSpeakToFriend(stop);
...

{
    ...
    for (
        list<Obj>::iterator i = l.begin(); i != l.end();
        i++)
    {  //现在加入CookingDish之后就不用担心了，如果看到是Cookingdish，就可以直接return1
        if (i->blockType == 3 && i->dish != 0)  //如果发现灶台里面有菜
        {
            ...
            else
            {
                ...
                if (PlayerInfo.dish >= OverCookedDish ||
                    PlayerInfo.dish == 0)  //如果拿到了黑暗料理，扔出去
                {
    ...
    int labelofcook)  //从label开始转一圈圈,int=1表示找到了可以用的灶台，int=2表示拿到菜了。
{
    ...
    while (isfind == 0)
    {
        flabel = flabel + 1;  // 2 3 4 5
        if (flabel == 5)
            flabel = 1;
        cout << "goto next label=" << flabel << endl;
        gotodest(Point(cookonly[flabel][0],
            cookonly[flabel][1]));  //{x,y,编号，朝向，label}
...
int pick_dish_in_block(Point food, int timelimit = 0)
{  // Point 是食物生成点
...
int whichfood()
{
    ...
    if (find(task_list.begin(), task_list.end(), SpicedPot) != task_list.end())
    {
      ...
      if (sizeraw >= 4 && mystorage.condimentList.empty() != TRUE &&
    getGameTime()<=510000)  { cout << "can make spice pot!" << endl; return
    SpicedPot;  //如果有调料，而且食材够，可以做香锅。
      }
    }
    */
...
void put_dest(
    Point dest,
    bool
    isdish)  //计算从当前位置到目标位置，需要的角度和距离,目标x+0.5,y+0.5才是中心点
{
...
int makefood(int food)  //传入目标的编号 whichfood
{
    ...
    else  //不做香锅
    {
        ...
        for (auto i : st)
        {
            cout << "try make :" << food << "    Dish :" << i.type
                << " step:" << i.stepsOfProcessed <<"  -pos"<<i.posList.begin()->x<<","<< i.posList.begin()->y << endl;
            if (i.stepsOfProcessed > maxstep)
                maxstep = i.stepsOfProcessed;
        }
        if (maxstep == 1)
        ...
        cout << "back to cook  label = "<<label << endl;
        ...
        list<Obj> l = MapInfo::get_mapcell(cookx, cooky);  //看看灶台里有啥
        cout << endl
            << "************** map info of cook before: ***************" << endl;
        for (list<Obj>::iterator i = l.begin(); i != l.end(); i++)
        {
            cout << "OBJ " << i->objType << " BLOCK:" << i->blockType
                << "   dish:" << i->dish << "   tool:" << i->tool << endl;
        }
        //
        Storage dishincook;
        ...
        for (list<Obj>::iterator i = l.begin(); i != l.end(); i++)
        {
            ...
            {
                // i的合成表
                if (find(rawlist.begin(), rawlist.end(), i->dish) ==
                    rawlist.end())  //如果在灶台里发现了多余的菜，拿走
                {
                    cout << "more food  " << i->dish << endl;
                    tPick(FALSE, Dish, i->dish);
                    tSleep(50);
                    tPut(0, PI, TRUE);
                    tSleep(50);
                }
                dishincook.add(i->dish, i->position);
            }
        }
        auto defl = dishincook.getDeficient(DishType(food));
        if (defl.empty() == FALSE)
        {
            cout << "deficient : " << *defl.begin() << endl;
            return 0;  //食物没齐，做菜失败
        }
    }
    ///////////////////////////////////////////////////////////////////////
    ...
    list<Obj> l = MapInfo::get_mapcell(cookx, cooky);  //看看灶台里有啥
    cout << endl
        << "************** map info of cook after: ***************" << endl;
    int dishnumber = 0;
    for (list<Obj>::iterator i = l.begin(); i != l.end(); i++)
    {
        cout << "TYPE:" << i->blockType << "   dish:" << i->dish
            << "   tool:" << i->tool << endl;
        if (i->dish >= 5 && i->dish <= 8)
            dishnumber++;
    }
    ...
    tSleep(50);
    smallmove(posnear.x + 0.5, posnear.y + 0.5);
    movex = sgn((int)cookpos.x - (int)PlayerInfo.position.x);
    movey = sgn((int)cookpos.y - (int)PlayerInfo.position.y);
    cookdir = dir[movey + 1][movex + 1];  //计算最后的朝向
    move_dir(cookdir, 0);                 //调整朝向
    tUse(0, 0, 0);                         //多试一次呗
    ...
    for (list<Obj>::iterator i = l.begin(); i != l.end(); i++)
    {
        if (i->blockType == 3 && i->dish == CookingDish)
            return 1;  //在做了在做了
    }
...
int commitTask()
{
    ...
    if (find(task_list.begin(), task_list.end(), PlayerInfo.dish) !=
        ...
        (PlayerInfo.dish >= SpicedPot && PlayerInfo.dish <= SpicedPot6 &&
            find(task_list.begin(), task_list.end(), SpicedPot) !=
            task_list.end()))
    {
        //手上的菜品在任务清单里面
        ...
        {
            cout << "high score dish get condiment  " << findres->second.Score
                << endl;
            mystorage.updatestorage();
            getcondiment(0);
        }
       ...
       //tSpeakToFriend(stop);
        ...
        if (PlayerInfo.dish != 0)  //手上还有菜，说明任务超时了，把菜放回原来的地方
        {
            ...
            if (PlayerInfo.dish != 0)
            {
                ...
                sdishinfo << 'd' << " " << PlayerInfo.dish << " " << (int)save.x << " "
                    << (int)save.y;
                cout << "send" << sdishinfo.str() << endl;
                tSpeakToFriend(string(sdishinfo.str()));
                tPut(0, 0, TRUE);
                tSleep(50);
                if (PlayerInfo.tool == Condiment)
                {
...
int findbestfoodgen(int timelimited = 0)  //传入cookedtime
{
    ...
    else
    {
        ...
        for (list<DishType>::iterator l = tklist.begin(); l != tklist.end();
            l++)  //用任务列表计算性价比
        {
            int i = *l;
            ...
            if (i != SpicedPot && i >= 1 && i <= DarkDish)
            {
                ...
                if (ls.empty() == FALSE)
                {
                    ...
                    {
                        r_value =
                            ((maketime + 5000) * sqrt(sumdis)) / (findres->second.Score);
                    }
                    else
                    {
                        r_value = sumdis / (findres->second.Score);
...
void mainswitch(int nextstate)
{
    ...
    case 0:  //自己的灶台正在做，这时候应该继续找食材再回来看。
        cout << "my cook is tUsed" << endl;
        state = 1;
        break;
    case 1:  //这个灶台有别人正在用，找第二近的灶台
        cout << "this cook is being tUsed" << endl;
        //如果返回2说明手里有菜可以提交了
...
void play()
{
    try
    {
        ...
        if (myround == 0)
        {
            ...
            gotodest(Point(cookonly[labelofcook][0],
                cookonly[labelofcook][1]));  //{x,y,编号，朝向，label}
            int nextstate =
                throw_darkdish(labelofcook);  //到达灶台后，先检查有无黑暗料理
        ...
        if (state == 0)
        {
            ...
            if (bestfoodgen <= 0 || bestfoodgen > 8)
                bestfoodgen = rand() % 8 + 1;
 ...
 //不用确保朝向，在throw里有了
        ...
        if (state == 1)
        {
            ...
            while (1)
            {
                //正常搬运食材
                int bestfoodgen =
                    findbestfoodgen(cooking[2]);  //和之前一样，参数是有时间限制的
                Point foodpos(foodgen[bestfoodgen][1], foodgen[bestfoodgen][2]);
                ...
                // gotodest(Point(cooklabel[labelofcook][0],
                // cooklabel[labelofcook][1]));  //{x,y,编号，朝向，label}
                //随机放在周围四格，防止被一锅端
                // tPut(1, angle_4[cooklabel[labelofcook][3]], TRUE);这是原来的定点投放。
                int randput = rand() % 4 + 1;
                ...
                if (cooking[4] < getGameTime() + 5000 ||
                    cooking[4] - getGameTime() > 100 * 60 * 1000)
                {  //
            ...
            while (wait)  //等待收菜
            {
                ...
                for (list<Obj>::iterator i = l.begin(); i != l.end(); i++)
                {
                    ...
                    if (i->dish != CookingDish && i->blockType == 3)
                    {
            ...
            state = 0;
            tSleep(50);
            cout << "get dish:" << PlayerInfo.dish << "  dest dish:" << cooking[0]
                << endl;
            if (PlayerInfo.dish == 0)
            {
                cout << "try again" << endl;
            ...
            if (PlayerInfo.dish == cooking[0] ||
                (cooking[0] == SpicedPot && PlayerInfo.dish >= SpicedPot &&
                    PlayerInfo.dish <= SpicedPot6))
            {
                cout << "success get dish" << endl;
                if (cooking[0] >= TomatoFriedEggNoodle || cooking[0] == CookedRice ||
                    (cooking[0] == TomatoFriedEgg &&
                        find(task_list.begin(), task_list.end(), PlayerInfo.dish) !=
                        task_list.end()))
                {
            ...
            cooklabel[label][4] = 0;
            if (state ==
                0)  //如果刚刚的结果是state==0，先在灶台边上再看一眼能做的菜。
            {
